...
const RolesView = () => {
  ...
  console.log("Extracting queries from RolesView!");
  ...
  function searchByInput() {
    ...
    var divs = Array.prototype.slice.call(
      document.querySelectorAll("#rolesList .list-card")
    );
        ...
        <div className="mt-3">
          {rCount.length === 1 && rCount !== 1 && (
            <label className="item-count pl-1">
              {data.getRoleCount.totalCount + " items"}
            </label>
          )}
          ...
          <div className="mt-3 mb-3 limit-scroll-four" id="rolesList">
            {data.getAllRoles.edges[0].node.roles && data.getAllRoles.edges[0].node.roles.map((value, index) => {
              return (
